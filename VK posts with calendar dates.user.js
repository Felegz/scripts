// ==UserScript==
// @name            VK posts with calendar dates
// @name:en         VK posts with calendar dates
// @version         0.1
// @description     Фильтрует в VK.COM только посты с датами и скрывает все остальные.
// @copyright       This script is 99% BASED on "VK posts filter" userscript by RALETAG. Source: http://greasyfork.org/scripts/23100/feedback
// @author          Felegz
// @supportURL      what
// @include         *://vk.com/feed
...
(function() {
    ...
    var showheader = true, // true - оставлять заголовок поста, false - полностью скрывать пост
        ...
        //regexp = new RegExp('(' + ((aps + '|' + trash + '|' + urlads + '|' + politic).replace(/у/g,'(у|y)').replace(/е/g,'(е|e)').replace(/х/g,'(х|x)').replace(/а/g,'(а|a)').replace(/р/g,'(р|p)').replace(/о/g,'(о|o)').replace(/с/g,'(с|c)').replace(/ /g,'([\\s]+)')) + ')', 'i'),
        regexp = new RegExp(/\d.(января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря)/i),
